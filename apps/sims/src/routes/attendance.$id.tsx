...
type AttendanceStateMark = "present" | "absent" | "late" | "excused";

interface AttendanceCorrectionItem {
  id: string;
  state: AttendanceStateMark;
  createdAt: string;
}

interface AttendanceEntryItem {
  id: string;
  studentId: string;
  state: AttendanceStateMark;
  corrections?: readonly AttendanceCorrectionItem[];
}

function AttendanceDetailPage() {
  const { occurrence, entries, roster } = Route.useLoaderData();
  ...
  const initialMarks: Record<string, AttendanceStateMark> = {};
  for (const entry of entries) {
    initialMarks[entry.studentId] = entry.state as AttendanceStateMark;
  }
  ...
  const canCorrect = ((permissions ?? []) as readonly string[]).some(
    (p) =>
      p === "attendance:correct:school" || p === "attendance:correct:assigned",
  );
  ...
  const getEffectiveState = (entry: AttendanceEntryItem) => {
  ...
  async function handleFinalize() {
    ...
    if (
      !window.confirm(
        "Are you sure you want to finalize this attendance register? This will freeze the register and block further ordinary marking. Corrections will require explicit authorization.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setMessage(
        err instanceof Error ? err.message : "Failed to finalize register.",
      );
    } finally {
      setFinalizing(false);
    }
    ...
    <PageShell application="SLGS S.I.M.S." eyebrow="Confidential register">
      <main className="sims-admin max-w-4xl">
          ...
          <div className="flex justify-between items-start flex-wrap gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                Register ID:{" "}
                <code className="font-mono text-xs">{occurrence.id}</code>
              </p>
            </div>
        ...
        <section
          ...
          className="p-4 border rounded-lg bg-card shadow-sm grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm"
        >
          ...
          <div>
            ...
            <span
              className="font-semibold text-xs font-mono truncate block"
              title={occurrence.recorderUserId}
            >
        ...
        >
          <h2 id="entries-heading" className="text-lg font-medium mb-4">
            Student Marks
          </h2>

          {entries.length === 0 ? (
            <p className="text-muted-foreground py-4 text-center">
              No attendance marks recorded. Click "Save Changes" above to record
              initial marks.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {entries.map((entry: AttendanceEntryItem) => {
                    ...
                    return (
                      <tr key={entry.id} className="hover:bg-muted/30">
                        ...
                        <td className="p-3 text-center">
                          <span
                            className={`capitalize px-2 py-0.5 rounded text-xs font-semibold ${hasCorrection ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}`}
                          >
                            {effective}
                          </span>
                        </td>
                        <td className="p-3 text-center">
                          {isFinalized ? (
                            <div className="flex justify-center gap-2 items-center">
                              ...
                              {hasCorrection && entry.corrections && (
                                <Link
                                  to="/attendance/$id/corrections"
                                  params={{ id: occurrence.id }}
                                  search={{ entryId: entry.id }}
