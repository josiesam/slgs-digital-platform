...
export const Route = createFileRoute(
  ...
  component: AccessClubsPage,
});

export function AccessClubsPage() {
  ...
  async function refresh(task: () => Promise<unknown>, success: string) {
    ...
    try {
      ...
      setFeedback("Club operation failed. Check authorization and key uniqueness.");
    } finally {
      setPending(false);
    }
  }
  ...
  const handleUpdateClub = (event: FormEvent<HTMLFormElement>, id: string) => {
    ...
    const status = String(data.get("status")) as "active" | "inactive" | "archived";
    if (
      status !== "active" &&
      !window.confirm(
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        <div>
          <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-wider mb-1">
            ...
            <span>/</span>
            <span className="text-foreground font-semibold">Clubs & Societies</span>
          </div>
          <h1 className="text-2xl font-serif font-bold text-foreground">Club Scope Management</h1>
          <p className="text-sm text-muted-foreground">
            Manage student societies, club scopes, and assignment-bound editorial supervision.
          </p>
        </div>
          ...
          >
            <div className="flex items-center justify-between border-b border-border pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-serif font-bold text-base text-foreground">{club.name}</h3>
              </div>
              <span
            ...
            <div className="space-y-3 text-xs">
              ...
              <label className="block space-y-1">
                <span className="font-medium text-foreground">Lifecycle Status</span>
                <select
                  name="status"
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-serif font-bold text-foreground">Create Club Scope</h2>
              <button
