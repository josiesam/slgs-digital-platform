...
export function AccessRolesView({
  ...
  async function refresh(task: () => Promise<unknown>, success: string) {
    ...
    try {
      ...
      setFeedback("Role action failed. Verify closed CMS permission catalogue strings.");
    } finally {
      setPending(false);
    }
  }
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1600px] mx-auto">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        <div>
          ...
          <h1 className="text-2xl font-serif font-bold text-foreground">Roles & Permissions</h1>
          <p className="text-sm text-muted-foreground">
            CMS role definitions, catalogue permissions, and explicit scope assignments.
          </p>
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-base font-semibold text-foreground">CMS Role Definitions</h2>
            ...
            >
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="font-serif font-bold text-sm text-foreground">{role.name}</span>
                  <span
                ...
                <p className="text-xs font-mono text-muted-foreground mb-1">{role.key}</p>
                {role.systemManaged ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-serif font-bold text-foreground">Create Custom CMS Role</h2>
              <button
            ...
            <form onSubmit={handleCreateRole} className="space-y-4 text-xs">
              ...
              <label className="block space-y-1">
                <span className="font-semibold text-foreground">Permissions (Catalogue Strings)</span>
                <textarea
                  name="permissions"
                  required
                  rows={3}
                ...
                <span className="text-[10px] text-muted-foreground block">
                  Comma-separated permission identifiers from the closed CMS catalogue.
                </span>
              </label>

              <label className="block space-y-1">
                <span className="font-semibold text-foreground">Scope Dimensions</span>
                <input
                  name="scopes"
                  placeholder="club"
                  className="w-full p-2 border rounded-md bg-background"
      ...
      {showAssignModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h2 className="text-lg font-serif font-bold text-foreground">Assign CMS Role</h2>
              <button
            ...
            <form onSubmit={handleAssignRole} className="space-y-4 text-xs">
              <label className="block space-y-1">
                <span className="font-semibold text-foreground">Target Member</span>
                <select name="member" required className="w-full p-2 border rounded-md bg-background">
              ...
              <label className="block space-y-1">
                ...
                <select name="role" required className="w-full p-2 border rounded-md bg-background">
                  {dashboard.roles
                    .filter((r) => r.active)
                    .map((r) => (
              ...
              {dashboard.clubs.length > 0 && (
                <label className="block space-y-1">
                  <span className="font-semibold text-foreground">Club Scope (If Required)</span>
              ...
              <label className="block space-y-1">
                <span className="font-semibold text-foreground">Audit Reason</span>
                <textarea
                  name="reason"
                  required
                  rows={2}
