...
export function EditorialKanbanView({
  ...
  const [newType, setNewType] = useState<"page" | "article" | "event" | "announcement" | "gallery">("article");
  const [newTitle, setNewTitle] = useState("");
  const [newSlug, setNewSlug] = useState("");
  const [newSummary, setNewSummary] = useState("");
  const [newBody, setNewBody] = useState("");
  const [newClubId, setNewClubId] = useState<string>(dashboard.clubs[0]?.id ?? "");
...
function KanbanCard({
  ...
  const revisionLabel = latestRev?.revisionLabel ?? `1.${item.currentRevision - 1}`;
  const snapshotId = item.currentSnapshotId ?? latestRev?.snapshotId;
  const baseSnapshotId = item.currentBaseSnapshotId ?? latestRev?.baseSnapshotId;
  ...
  return (
    <article className="p-4 rounded-lg border border-border bg-card shadow-sm space-y-3 hover:shadow-md hover:border-[#42245f]/30 transition-all text-xs">
      ...
      <div>
        ...
        <p className="text-[11px] font-mono text-muted-foreground mt-0.5">{item.canonicalPath}</p>
      </div>
      ...
      <div className="rounded-md bg-muted/40 p-2 text-[10px] space-y-1 font-mono border border-border/40">
        ...
        {snapshotId && (
          <div className="flex items-center justify-between">
            ...
            <span className="truncate max-w-[120px] text-foreground" title={snapshotId}>
              {snapshotId}
            </span>
          </div>
        )}
        {baseSnapshotId && (
          <div className="flex items-center justify-between">
            ...
            <span className="truncate max-w-[120px] text-foreground" title={baseSnapshotId}>
              {baseSnapshotId}
            </span>
          </div>
        )}
