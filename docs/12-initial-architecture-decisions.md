...
- CMS Administrator is the CMS-only operational super-role under ADR-040. It
  can perform every ordered editorial task, including its own reviewed and
  approved workflow, without receiving S.I.M.S. authority.
- Plate JSON is the canonical rich-text representation for new CMS body edits
  under ADR-041; derived plain text remains for backwards compatibility and
  safe non-HTML fallbacks.
