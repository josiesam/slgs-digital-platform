...
`apps/web` is an anonymous TanStack Start application and remains independently deployable from CMS. It imports `@slgs/public-content`, not `@slgs/cms-domain` or `@slgs/auth`. Shared presentation tokens remain in `@slgs/ui`; Web-specific composition remains in `apps/web`.
...
The `slgs_web` runtime role receives `USAGE` on `public_content` and `SELECT` on its views. It retains no CMS or identity schema access and no public-content mutation privilege. The CMS runtime role receives no implicit access to the public schema.
...
Unit coverage checks DTO privacy, bounded caching, public route families and SEO projection. The live `phase-1d.sql` verification inserts synthetic content across every CMS state for every public type, proves each view exposes only its published row, proves unpublication removes an article, checks projection columns and verifies the Web database role is read-only and isolated from CMS and identity data.
