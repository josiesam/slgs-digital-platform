...
Students are administrative records rather than authenticating identities. Phase 2B stores students, staff, classes, subjects and academic sessions in a private `sims` schema available only to the `slgs_sims` runtime role. A staff record may explicitly link to an existing Better Auth identity; no domain record stores credentials. Typed services validate input, authorize the session actor, apply assignment scope to SQL queries and transact mutations with the existing append-only security audit. CMS/Web have no schema access or public projection. See `docs/20-phase-2a-sims-identity-administration.md` and `docs/22-phase-2b-sims-core.md`.

Phase 2C attendance will preserve occurrence-time academic session/class and roster context rather than deriving history from a student's mutable current class. It requires no term/calendar subsystem by default. Corrections preserve immutable superseded evidence, and every write retains authenticated identity attribution with an optional explicit staff link. Attendance implementation remains conditional on the policy decisions in `docs/24-phase-2c-attendance.md`.
